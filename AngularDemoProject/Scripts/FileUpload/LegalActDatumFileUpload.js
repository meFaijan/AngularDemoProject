...
function ActDatum_loadActDatum_uploadedFiles() {
    var leg05uin = $("#leg04uin").val();
    ...
    var url1 = _CurrentControllerNameActDatum1;
    var url2 = _CurrentControllerNameActDatum2;

    $('#ActDatum_uploadedFiles').load(url1, function (response, status, xhr) {
        if (status == "error") {
...
$(document).ready(function () {
    ...
    loadActDatum_uploadedFiles();
    $("#ActDatum_FileSubmit").on('click', function () {
        //if (!canContinueNegativeList()) { return; }
       
        var $leg01uin = $("#leg04uin").val();

        console.log($leg01uin);
        var countFiles = $("#ActDatum_MainFile")[0].files.length;

        if (countFiles == 0) {
            return alert("Please Select the file for Upload.");
        }
        $("#ActDatum_FileSubmit").prop("disabled", true);
        showLoading();

        var uploadCount = 0;
        jQuery.each(jQuery('#ActDatum_MainFile')[0].files, function (i, file) {
            var fileSize = file.size / 1024 / 1024;
            if (fileSize > 3) {
                $.toaster("File Size for File <b>" + file.name + "</b> is greater than upload limit for file . Max Size :3mb ", "Error", "danger");
                jQuery('#ActDatum_MainFile').val('');
                $("#ActDatum_FileSubmit").prop("disabled", false);
        ...
        jQuery.each(jQuery('#ActDatum_MainFile')[0].files, function (i, file) {
            ...
            //});
            $.ajax({
                url: _CurrentControllerNameActDatum3,//string variable
                type: 'POST',
                data: $formData,
                contentType: false,
                processData: false,
                success: function (Data) {
                    ...
                    if (uploadCount >= countFiles) {
                        $("#ActDatum_MainFile").val("");
                        $("#ActDatum_FileSubmit").removeAttr("disabled");
                        hideLoading();
                    }
                }, error: function ($err) {
                    console.log($err);
