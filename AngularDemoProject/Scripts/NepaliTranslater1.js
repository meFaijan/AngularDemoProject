    ...
    for (i = 0; i < words.length; i++) {
        ...
        if (sW["smartconverter_on"] == "true") {
            if (hasSW(words[i]));
                ...
                if ((ec_0 == 'a' || ec_0 == 'e' || ec_0 == 'u') && ec_1 == 'h' && ec_2 == 'c') words[i] = words[i].substring(0, words[i].length - 3) + "chh" + ec_0;
                else if (ec_0 == 'y') words[i] = words[i].substring(0, words[i].length - 1) + "ree";
                ...
                if (ec_0 == 'i' && !isVowel(ec_1)) words[i] = words[i].substring(0, words[i].length - 1) + "ee"
            }
        }
        subs = words[i].split("/");
        for (j = 0; j < subs.length; j++) if (subs[j].length != 0) rVal += getAllUnicode(subs[j], html);
        rVal += " "
    }
    return rVal
}
...
function getUnicode(t, ll, html) {
    ...
    var stopPos = 0;
    var ar = t.split("+");
    if (ll && ar && ar.length > 1 && sW["smartconverter_on"] == "true" && ar[ar.length - 1] == "2381") stopPos = 1;
    if (ar) for (k = 0; k < (ar.length - stopPos); k++) if (ar[k].length > 0 && !html) u += String.fromCharCode(ar[k]);
...
function translate(source, html, smart) {
    var input = source;
    var beginIndex = 0;
    var endIndex = -1;
...
//translate(Form1.ascii.value, true || false, true || false)
console.log(translate("hello ram", false, true))
$(document).ready(function () {
    $(document).on('change', '.translateToNepali', function (e) {
        var t = $(this).data('target');
        $('#' + t).val(translate($(this).val(), false, true));
        unicode2preeti(t,$('#' + t).data('target'));
    });
...
function TranslateBulk() {
    $('.translateBulk').each(function (item, index) {
        var t = $(this).data('target');

        var pretiFontValue = $('#preeti_' + t).val();
        var nonPretiFontValue = $('#' +t).val();
        //console.log("pretiFontValue", pretiFontValue);
        //console.log("nonPretiFontValue", nonPretiFontValue);
      if (pretiFontValue == undefined || pretiFontValue == null || pretiFontValue == null ||  pretiFontValue.length <= 2) {
           // console.log("pretiFontValue1", pretiFontValue);
            $('#' + t).val(translate($(this).val(), false, true));
            unicode2preeti(t, $('#' + t).data('target'));           
        }
    if (nonPretiFontValue == undefined || nonPretiFontValue == null || nonPretiFontValue.length <= 2) {
           // console.log("nonPretiFontValue1", nonPretiFontValue);
            $('#' + t).val(translate($(this).val(), false, true));
            unicode2preeti(t, $('#' + t).data('target'));
        }
