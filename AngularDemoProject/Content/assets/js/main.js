const ANTSNumber = {
    ...
    formatCur: function ($amt) {
        //todo: change it for thousand separator function
        $amt = $amt.replace(/,/g, '');
        var ret = 0;
        if (isNaN($amt) || $amt == 0) { return ret; }
        return parseFloat($amt);
...
//add  custom general function
function onlyNum($obj) {
    //console.log($obj.val());
    $obj.val($obj.val().replace(/\D/g, ''));
}

function applySelect($selector) {
    //return;
    $('.ch_vou03led05code').each(function (index, item) {
        //console.log($(this).val());
        $(this).prev().val($(this).val());
    });

    $('.ch_vou03led05code-ajax').each(function (index, item) {
        $(this).prev().html(Select2Helper.getOption($(this).val(), $(this).data('text'), true));
    });

    $('.ch_vou03led05code_target').each(function (index, item) {
        debugger;
        $array = $(this).val().split(',');
        var target_id = "#" + $(this).data('target');
        $.each($array, function (index, item) {
            console.log(target_id);
            $(target_id + ' option[value = ' + item + ']').attr("selected", "selected");
        });
    });

    $selector = $selector || "select.search-select";
    $($selector).select2({
        placeholder: "Select ",
        allowClear: true
    });
    $("select.search-select-ajax").each(function (index, item) {

        var data_value = $(this).data("value");
        var data_title = $(this).data("title");
        if (data_value != null) {
            var Result = '<option value="' + data_value + '">' +
                data_title +
                '</option>';
            $(this).html(Result);
        }
...
$(document).ready(function () {
    //for edit page assign value to previous sibling
    applySelect();
    $('.date-picker').datepicker({
        autoclose: true,
        format: "yyyy-mm-dd"
    });
    $('.triggerSearch').on('dblclick', function (e) {
        $('.popupSearch').trigger('click');
    });
    $('.triggerSearch').on('keyup', function (e) {
...
function htmlOption(val, display, isSelected) {
    return '<option value="' + val + '" ' + ((isSelected || false) ? "selected='selected'" : '') + ' > ' + display + '</option>';
}
...
var Main = function () {
    ...
    var setSearchMenu = function () {
        ...
        $('.menu-search *[type=submit]').on('click', function (e) {
    ...
    var runNavigationMenu = function () {
        if ($("body").hasClass("single-page") == false) {
            ...
            $('.main-navigation-menu > li a').on('click', function () {
                ...
                if ($(this).prev()) {
                    $(this).prev().children('li.open').removeClass('open').children('ul').slideUp(200, function () {
                        if (mainNavigation.height() > $(".main-navigation-menu").outerHeight()) {
                            mainNavigation.scrollTo(0, 300, {
                                onAfter: function () {
                                    if ($body.hasClass("isMobile") == false) {
                ...
                if ($(this).next()) {
                    $(this).next().children('li.open').removeClass('open').children('ul').slideUp(200, function () {
                        if (mainNavigation.height() > $(".main-navigation-menu").outerHeight()) {
                            mainNavigation.scrollTo(0, 300, {
                                onAfter: function () {
                                    if ($body.hasClass("isMobile") == false) {
    ...
    var limitNumber = function () {
        ...
        //$(document).on('keydown', '.numberWithMinusSlash', function (e) {
        //	// Allow: backspace, delete, tab, escape, enter and . as well as - and /
        ...
        $(document).on('paste', '.number', function (e) {
            //console.log('paste called : ->', $(this).val());
            $(this).val($(this).val().replace(/\D+/g, ''));
            //onlyNum($(this));
        });
    };

    var limitNumberExt = function () {
        $(document).on('keydown', '.numberExt', function (e) {
            // Allow: backspace, delete, tab, escape, enter and .
            if ($.inArray(e.keyCode, [46, 8, 9, 27, 13, 110, 190]) !== -1 ||
                // Allow: Ctrl+A, Command+A
                (e.keyCode === 65 && (e.ctrlKey === true || e.metaKey === true)) ||
                ...
                (e.keyCode >= 35 && e.keyCode <= 40) ||
                // Allow: /,/[numpad],-,-[numpad]
                ($.inArray(e.keyCode, [111, 191, 109, 173])) ||
                //alow Ctrl + v
                (e.keyCode === 86 && (e.ctrlKey === true || e.metaKey === true))) {
                // let it happen, don't do anything
                return;
...
function showLoading() {
    $('#ajaxLoaderDiv').removeClass('hidden').fadeIn(300);
    $('#ajaxLoaderDivBackground').removeClass('hidden').fadeIn(300);
}
function hideLoading() {
    $('#ajaxLoaderDiv').fadeOut(300);
    $('#ajaxLoaderDivBackground').fadeOut(300);
}
$(document).ready(function () {
    //for edit page assign value to previous sibling
    $('.ch_vou03led05code-ajax').each(function (index, item) {
